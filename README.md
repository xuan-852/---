...
| 📱 **小程序课表网格** | 整周可见，左右滑动切换周数，课程卡片着色 | ✅ 完成 |
...
| # | 问题 | 根因 | 修复 |
|---|------|------|------|
| 1 | **手机端一直显示"未绑定"** | ngrok 免费版在移动端返回 HTML 拦截页，小程序解析失败 | 所有 `wx.request` 添加 `ngrok-skip-browser-warning: true` 请求头绕过 |
| 2 | **课表列错位（整体左移一天）** | 服务端 `dayMap` 中`'星期一': 0`，导致周一存为周日 | 修正为 `'星期一': 1, '星期二': 2, ... '星期日': 0`，符合 DB day 规范 |
| 3 | **周日课程不可见** | 前端 `_buildCards` 假设 `day=1..7`，但 DB 中周日为 `day=0` | 新增 `_normalizeDay(0 → 7)` 归一化映射 |
| 4 | **首页今日课程过滤错误** | 用 `getDay() \|\| 7` 映射周日，而 DB 周日为 0 | 直接比较 `c.day === dbDay`（两者格式一致）

#### ✨ 功能改进

| # | 改进 | 描述 |
|---|------|------|
| 1 | **课表整周网格布局** | 从横向滚动改为 flex 网格，整周可见，无需滑动即可看到周一至周日 |
| 2 | **滑动切换周数** | 左右滑动（>40px）切换上一周/下一周 |
| 3 | **设置页诊断面板** | 显示服务器地址、原始响应、错误详情，增加"强制检查"按钮 |
| 4 | **状态检查降级** | `settings.checkStatus()` 在 API 封装失败时自动降级到直连 `wx.request` |

#### 🔧 技术改进

- 所有请求头统一添加 `ngrok-skip-browser-warning: true`（app.js / api.js / settings.js 共 7 处）
- `getUserStatus()` 移除缓存回退，防止过期"未绑定"缓存误导
- `bindAccount()` 绑定前清除旧状态缓存
- 课表服务端 `saveScheduleToDB` dayMap 修正，前端 `_normalizeDay` 统一处理
